...
export class BrowserManagerService {
    ...
    private async getProxiedResponse(
      ...
      if (this.bestProxies[host].best.length === 0) {
        this.bestProxies[host].best = getRandomsFromArray(proxyAdressess, 50)
      }
  
      const proxiesForRequest = Array.from(new Set([...getRandomsFromArray(this.bestProxies[host].best, 20), ...getRandomsFromArray(proxyAdressess, 10)]))
      ...
      try {
        const result = await Promise.any(agents.map(async ({ proxyAddress, agent }) => {
          ...
          if (browseContext.isValidResponse(response)) {
            ...
            while (this.bestProxies[host].best.length > 100) {
